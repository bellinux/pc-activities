...
content_sections:
  ...
  - title: "Composición del Código"
    ...
    icon: "code"
    content: |
      Este código es un ejemplo perfecto de eficiencia. `Al iniciar` prepara la variable `estado` y colorea con negro el dibujo LED. Luego, el programa espera sin hacer nada. El bloque de evento `cuando el nivel de ruido detecta aumento repentino` lo es todo: primero, usa la lógica de 'interruptor' (`estado = no estado`) para invertir el estado actual. Inmediatamente después, un bloque `si... sino...` lee el *nuevo* estado de esa misma variable y enciende o apaga la luz. Todo sucede en un instante, solo cuando es necesario.
