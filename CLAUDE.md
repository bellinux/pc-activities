...
| `tools/make-slides.py` · `ppt/` | **Generatore di presentazioni** (copia autonoma di quello del sito di Protobject, 19-09-2026) e le 17 presentazioni `ppt/<slug>.es.pptx`, scaricabili dal visore (pulsante «Presentación»). `python tools/make-slides.py [--force] [frammento…]`. La slide del programma fotografa `app.protobject.com/generate?equivalent-…`, che legge il `.ptj` **pubblicato**: se cambia un programma, prima si pubblica, poi si rigenera. Cache in `tools/media-cache/` (non versionata). |
...
- **Il sito di Protobject (`../../../new-static-site`, protobject.com) NON si tocca (19-09-2026).** I suoi `.md` sono le versioni rifinite per il sito; i nostri 17 sono le versioni per l'equivalenza con micro:bit e da ora vanno per conto loro. Dal sito si può solo prendere ispirazione. Importato il 19-09-2026: la riga dei materiali (cartone, forbici, elastici) nei 17, `energia` dove il 16.4 nomina la variabile, quattro video (02.2 e 16.5 accorciati, 17.1 e 19.1 girati) e quattro disegni con contenuto migliore (17.2 `c-2d-matrix` riempito per righe come il programma, 22.1 `c-advanced-time-logic` con `ultimoPulso`/`tiempoTranscurrido`, 22.1 `d-interval-formula.es.svg` in spagnolo, 03.1 `d-multiple-visualizer` vettoriale). **Non** importati i disegni che il sito ha solo convertito in testo vero: pesi più leggeri, scritte ruotate raddrizzate, alcune illeggibili («DISPARA» del 14.1); i nostri a tracciati sono fedeli all'originale.
