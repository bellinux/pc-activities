...
- **`pipeline-equivalence/`**: pipeline **ESEGUITA** per le 17. **Per rieseguirla quando cambiano attività → segui `pipeline-equivalence/prompt.md`** (`reporte/scripts/pipeline_llm.js` + `run_all.py` env `pdg-vela`). **Nota:** il tool `Workflow` è bloccato da un bug dell'hook di permessi ("script contains control characters") → si replicano i prompt dello script con il tool `Agent`, uno per attività.
