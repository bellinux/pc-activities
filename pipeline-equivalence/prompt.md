...
- **PDG · 4 regole:**
  ...
  - **(C) Suono per INTENZIONE** (non per timbro né dispositivo): `PlayTone` **solo** se il pitch è **controllato/variabile** (xilofono — il pitch esce dall'inclinazione); `EmitSound` se è un suono **generico a evento** (battito, bip, percussione, o tono a nota **fissa** usato come segnale — l'ex "drum" va **qui**); `PlaySound` se è un **clip/melodia con nome** (allarme, risata…). Encoding Hz≡nota = stesso pitch (plumbing). **Criterio:** pitch calcolato/variabile → `PlayTone`; pitch fisso/incidentale → `EmitSound`. Niente `PlayDrum`. La `tabla_ontologica.json` porta una **nota del "quando"** su ciascuno dei tre.
  - **(D) Aritmetica = opzione B:** operatore **SÌ** (campo `op` ∈ `+ − × ÷ constrain`), costante **NO**. Quindi `+1000` ≡ `+60`; ma `×` ≠ `+`.
